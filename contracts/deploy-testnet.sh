...
do
  ...
  near call "$CONTRACT" add_post '{"dao_id":1, "body":{"title":"Report title #2/'"$i"'", "description":"Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...Report description 2...", "attachments":["https://some_attachment.com", "https://some2_attachment.com"], "labels":["report-label", "gaming"], "metrics":{}, "proposal_id":1, "post_type": "Report", "report_version": "V1"}}' --deposit 0.01 --accountId "$ACCOUNT_ID"
  near call "$CONTRACT" add_post '{"dao_id":2, "body":{"title":"Proposal title #3/'"$i"'", "description":"Proposal description 3...", "attachments":[], "labels":[], "metrics":{}, "reports":[], "requested_amount": 10000, "post_type": "Proposal", "proposal_version": "V1"}}' --deposit 0.01 --accountId "$ACCOUNT_ID"
done
...
# Edit report
 ...
 near call "$CONTRACT" change_proposal_state '{"id":1, "state":{"dao_council_approved":true, "hom_approved":true, "coa_approved":true, "kyc_passed":true, "payment_executed":true, "report_provided":true}}' --accountId "$ACCOUNT_ID"
...
near view "$CONTRACT" get_follow_list '{"follow_type":"DAO", "account_id":"'$ACCOUNT_ID'"}'
near view "$CONTRACT" get_follow_list '{"follow_type":"Community", "account_id":"'$ACCOUNT_ID'"}'